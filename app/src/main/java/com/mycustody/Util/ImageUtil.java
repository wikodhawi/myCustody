...
public class ImageUtil {
	...
	public static void displayCircleImage(CircleImageView view, String path, ImageLoadingListener listener) {
		ImageLoader loader = ImageLoader.getInstance();
		try {
			loader.displayImage(path, view, DEFAULT_DISPLAY_IMAGE_OPTIONS, listener);
		} catch (OutOfMemoryError e) {
	...
	private static final DisplayImageOptions.Builder DEFAULT_DISPLAY_IMAGE_OPTIONS_BUIDLER = new DisplayImageOptions.Builder()
			...
			.showImageForEmptyUri(R.mipmap.ic_profile)
			.showImageOnLoading(R.mipmap.ic_profile)
			.showImageOnFail(R.mipmap.ic_profile).cacheOnDisk(true)
